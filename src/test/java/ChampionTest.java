...
public class ChampionTest {
    ...
    public void notNullCheck() {
        String lck = "LCK";
        assertThat(lck, notNullValue()); // 빈 값이 아니면 패스 성공
    }
    ...
    public void shouldNotErrorGetReference() {
        ...
        // 어떤 값이든 상관 없다. 값이 있으면 된다.
        // champlionList.get(6)을 하면 값이 없는 범위가 참조가 돼서 index out of bound exception이 남
    ...
    public void shouldChampionCountFive() {
        // Before annotation이 있어서 그 코드가 실행되고 test 코드가 실행되기 때문에
        // 이 메소드 안에 객체를 안 넣어 놔도 됨
        assertTrue(championList.size() == 5);
        assertThat(championList.size(), is(5));
        assertThat(championList, hasSize(5)); // 객체 자체를 주면 객체 크기 자체를 계산해줌
    }
    ...
    public void shouldHasPropertyPosition() {
        assertThat(championList.get(0), hasProperty("position", equalTo("메롱")));
    ...
    public void shouldTopChampionIsDarius() {
        ...
        Optional<Champion> filteredBottom = championList.stream()
                .filter(c -> c.getPosition().equals("바텀"))
                .findFirst();
        String champName2 = filteredBottom.get().getName();
        assertTrue(champName2.equals("베인"));
        assertThat("베인", is(champName2));

        Optional<Champion> filteredName = championList.stream()
                .filter(c -> c.getName().equals("르블랑"))
                .findFirst();
        String position = filteredName.get().getPosition();
        assertTrue(position.equals("미드"));
        assertThat("미드", is(position));
